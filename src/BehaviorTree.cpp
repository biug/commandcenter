...
BehaviorTreeBuilder* BehaviorTreeBuilder::Condition(EConditionMode ConditionMode, bool IsNegation,const sc2::Unit * unit)
{
	//根据输入参数创建不同条件节点
	Behavior* Cd;
	...
	case EConditionMode::IsSeeEnemy:
		Cd = Condition_IsSeeEnemy::Create(IsNegation,unit);
		break;

	case  EConditionMode::IsHealthLow:
		Cd = Condition_IsHealthLow::Create(IsNegation,unit);
		break;

	case EConditionMode::IsEnemyDead:
		Cd = Condition_IsEnemyDead::Create(IsNegation,unit);
		break;
