    ...
    const std::vector<std::vector<float>>& map_to_path)
{
	...
	std::cout << "currentDSP:" << map_to_path[start_point.x][start_point.y]<<std::endl;
...
void Pathfinding::SmartPathfind(const sc2::Unit* unit, const sc2::Point2D & target_position, CCBot & bot)
{
...
void Pathfinding::SmartRunAway(const sc2::Unit* unit, const int run_distance, CCBot & bot)
{
    Pathfinding p;
    std::vector<sc2::Point2D> move_path = p.DjikstraLimit(sc2::Point2DI(unit->pos.x, unit->pos.y),
        run_distance,
        bot.Map().GetDPSMap(unit->pos,run_distance));
    //SmartMove(unit, move_path[0], bot, false);
    //SmartMove(unit, move_path[1], bot, true);
    //SmartMove(unit, move_path[2], bot, true);

    bot.Map().drawLine(unit->pos, move_path[0]);
    bot.Map().drawLine(move_path[0], move_path[1]);
    bot.Map().drawLine(move_path[1], move_path[2]);
    bot.Map().drawLine(move_path[2], move_path[3]);
    bot.Map().drawLine(move_path[3], move_path[4]);
    bot.Map().drawLine(move_path[4], move_path[5]);
    bot.Map().drawLine(move_path[5], move_path[6]);
 //   Micro::SmartMove(unit, move_path[3], bot);
