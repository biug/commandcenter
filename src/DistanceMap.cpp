...
void DistanceMap::computeDistanceMap(CCBot & m_bot, const sc2::Point2DI & startTile)
{
    ...
    {
        const sc2::Point2DI & tile = fringe[fringeIndex];

        // check every possible child of this tile
        for (size_t a=0; a<LegalActions; ++a)
        {
            sc2::Point2DI nextTile(tile.x + actionX[a], tile.y + actionY[a]);
