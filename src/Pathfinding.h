...
public:
    ...
    std::vector<sc2::Point2D> DjikstraLimit(const sc2::Point2DI start_point, const int max_run_dist,
                                            const std::vector<std::vector<float>>& map_to_path);
    bool TestDjikstra();
    void SmartPathfind(const sc2::Unit * unit, const sc2::Point2D & target_position, CCBot & bot);
    void SmartRunAway(const sc2::Unit * unit, const int run_distance, CCBot & bot);
};
